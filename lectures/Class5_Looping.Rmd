...
climate_growth = rnorm(n=10, mean = 20, sd=2)

growth = rep(x=0, times=10)
for (i in 1:10) {
  growth[i] = climate_growth[i] + 1.8*fertilizer[i]**2-0.9*fertilizer[i]
}

plot(growth)
# or put together

crop = data.frame(growth=growth, fertilizer=fertilizer, climate_growth=climate_growth)
# now we can plot against fertilizer (and include climate effect as well)
ggplot(crop, aes(fertilizer, growth))+geom_point()+geom_point(aes(fertilizer, climate_growth), col="red")
